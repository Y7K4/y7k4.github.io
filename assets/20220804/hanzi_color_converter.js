...
function onColorChange() {
  ...
  const rGoal = parseInt(color.slice(1, 3), 16);
  const g = parseInt(color.slice(3, 5), 16) & 0xfd | 0x01;
  const b = parseInt(color.slice(5, 7), 16) & 0xfd | 0x01;
  const fullRangeR = [0x27, 0xa7, 0x67, 0xe7, 0x17, 0x97];
  const safeRangeR = [0xa7, 0x67, 0xe7, 0x17];
  const getClosest = (x, y) => Math.abs(y - rGoal) < Math.abs(x - rGoal) ? y : x;
  const rUnsafe = fullRangeR.reduce(getClosest);
...
function drawCanvas(color, hanzi) {
  ...
  ctx.fillText(hanzi, canvas.width / 2, canvas.height / 2);
